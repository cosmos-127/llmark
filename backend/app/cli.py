...
def evaluate_assertions(run_record: BenchmarkRun, assertions: list[str]) -> tuple[bool, list[str]]:
    ...
    for raw_ast in assertions:
        ...
        if not match:
            all_passed = False
            results.append(
                f"[ERROR] Invalid assertion format: '{ast}' (expected e.g. 'p95_ttft < 800' or 'goodput >= 95')"
            )
            continue
        ...
        if metric_key not in metrics_map:
            all_passed = False
            results.append(
                f"[ERROR] Unknown assertion metric: '{metric_key}'. Available: {list(metrics_map.keys())[:8]}"
            )
            continue
...
async def run_headless_benchmark(
    ...
    fail_under_goodput: float | None = None,
    ...
    print(
        f"   Workload: {config.workload_preset.value} | Concurrency: {config.concurrency} streams"
    )
    print(
        f"   Duration: {config.duration_seconds}s | Spend Cap: ${config.hard_spend_cap or 0.0:.2f}"
    )
    print("=======================================================\n")
    ...
    while execution.status in ("initializing", "running"):
        ...
        print(
            f"\r[RUNNING] Elapsed: {elapsed:.1f}s | Finished Reqs: {len(execution.metrics)}",
            end="",
            flush=True,
        )
    ...
    print(
        f"   * Requests Completed:   {run_record.completed_requests} / {run_record.total_requests}"
    )
    ...
    if fail_under_goodput is not None:
        if run_record.goodput_pct < fail_under_goodput:
            print(
                f"[FAILED] CI GATE: Goodput {run_record.goodput_pct:.1f}% is below required threshold {fail_under_goodput:.1f}%"
            )
            gate_failed = True
        else:
            print(
                f"[PASSED] CI GATE: Goodput {run_record.goodput_pct:.1f}% satisfies threshold >= {fail_under_goodput:.1f}%"
            )
...
def main():
    ...
    run_parser.add_argument(
        ...
        type=str,
        default="mock",
        help="Vendor name (mock, openai, anthropic, openai_compatible)",
    )
    run_parser.add_argument("--model", type=str, default="gpt-4o", help="Model identifier")
    run_parser.add_argument(
        "--preset", type=str, default="chat", help="Workload preset (chat, rag, code, etc.)"
    )
    run_parser.add_argument(
        "--concurrency", type=int, default=5, help="Number of concurrent workers"
    )
    run_parser.add_argument(
        "--duration", type=int, default=15, help="Benchmark duration in seconds"
    )
    ...
    run_parser.add_argument(
        ...
        type=float,
        default=None,
        help="Fail with exit code 1 if Goodput % is below this value",
    )
    run_parser.add_argument(
        "--assert",
        "-a",
        dest="assertions",
        action="append",
        help="Assertion rule e.g. 'p95_ttft < 800' or 'goodput >= 95' (can be specified multiple times)",
    )
    run_parser.add_argument(
        "--output-md", type=str, default=None, help="Output path for Markdown summary report"
    )
    run_parser.add_argument(
        "--output-pdf", type=str, default=None, help="Output path for executive PDF report"
    )
