"""Production-Grade Workload Prompt Presets for LLMark Benchmark Engine.

Each prompt preset is meticulously calibrated to stress-test specific hardware,
network, and algorithmic dimensions of LLM inference engines (TTFT, TPOT, ITL jitter,
KV cache reuse, guided grammar decoding, agentic tool calling, long-context attention,
and 429 rate limit ceilings).
"""

from app.models.schemas import WorkloadPreset

# ==============================================================================
# WORKLOAD 1: RATE LIMIT & QUOTA PROBING (~5 prompt tokens / 1-2 output tokens)
# Purpose: Micro-cost health call to probe RPM/TPM ceilings and 429 backoff handling.
# ==============================================================================
PROMPT_RATE_LIMIT_PROBE = "PING. Reply with exactly: PONG"


# ==============================================================================
# WORKLOAD 2: PREFILL SCALING & TTFT (~4,000 prompt tokens / 1-2 output tokens)
# Purpose: Isolate pure KV prefill computation latency and prompt tok/s without decode.
# ==============================================================================
_PREFILL_DOC_HEADER = """# HIGH-THROUGHPUT DISTRIBUTED INFERENCE ENGINE SPECIFICATION
...
PROMPT_PREFILL_TTFT = (
    _PREFILL_DOC_HEADER + "\n" + "\n".join(_PREFILL_LOG_SAMPLES * 6) + "\n" + _PREFILL_DOC_FOOTER
)


# ==============================================================================
# WORKLOAD 3: STREAMING DECODE & JITTER (~40 prompt tokens / ~800 output tokens)
# Purpose: Benchmark sustained decode tok/s, ITL jitter percentiles, and TPOT stability.
# ==============================================================================
PROMPT_DECODE_THROUGHPUT = (
    "Author an exhaustive, production-grade technical engineering guide on Designing Low-Latency Streaming Telemetry Systems "
    "at Massive Scale. Provide in-depth architectural explanations, concrete algorithmic implementations, and failure modes "
    "across five core domains: 1. Zero-copy ring-buffer memory management and kernel bypass (eBPF / io_uring); 2. Sliding-window "
    "stream aggregation and high-throughput lock-free data structures; 3. Adaptive backpressure and credit-based flow control "
    "mechanisms; 4. Fault-tolerant distributed consensus and sub-millisecond failover protocols; 5. Inter-Token Latency (ITL) "
    "jitter elimination and tail latency SLA governance. Write with rigorous technical depth and comprehensive operational insights."
)


# ==============================================================================
# WORKLOAD 4: REASONING & COT DEEP-DIVE (~300 prompt tokens / ~800 output tokens)
# Purpose: Deep multi-constraint optimization problem triggering extensive thinking phase.
# ==============================================================================
PROMPT_REASONING_COT = (
    "You are the Principal Infrastructure Architect designing a multi-tier distributed scheduling engine for a heterogeneous inference fleet. "
    "You have three node classes:\n"
    ...
    "Formulate the formal optimization problem, evaluate all partitioning combinations across clusters Alpha, Beta, and Gamma, "
    "prove the mathematical bottleneck on the critical path under queueing saturation (M/M/k model), and output the exact provably "
    "optimal resource assignment matrix with step-by-step reasoning."
)


# ==============================================================================
# WORKLOAD 5: AGENTIC TOOL & FUNCTION CALLING (~1,200 prompt tokens / ~150 output tokens)
# Purpose: Enterprise multi-tool schemas measuring tool call latency & argument validity.
# ==============================================================================
PROMPT_AGENTIC_TOOL_CALLING = """You are an Autonomous Site Reliability Engineering (SRE) Agent for a global cloud infrastructure platform.
...
# ==============================================================================
# WORKLOAD 6: CODE GENERATION & SYNTAX STREAM (~1,500 prompt tokens / ~800 output tokens)
# Purpose: Measure code completion throughput, token jitter during syntactic formatting.
# ==============================================================================
PROMPT_CODE_GENERATION = '''You are a Principal Systems Engineer implementing a mission-critical, high-concurrency Rate Limiting and Token Bucket engine in Python 3.12 for an asynchronous LLM gateway.
...
# ==============================================================================
# WORKLOAD 7: ENTERPRISE RAG SYNTHESIS (~3,500 prompt tokens / ~400 output tokens)
# Purpose: Ingest multi-source retrieved knowledge passages, resolve conflicts, and synthesize.
# ==============================================================================
_RAG_SYNTHESIS_DOCS = """---
...
PROMPT_RAG_SYNTHESIS = _RAG_SYNTHESIS_DOCS * 2 + "\n" + _RAG_SYNTHESIS_TASK


# ==============================================================================
# WORKLOAD 8: LONG-CONTEXT & NEEDLE RETRIEVAL (~16,000 prompt tokens / ~300 output tokens)
# Purpose: Massive 16k prompt measuring memory pressure, KV scaling & needle precision.
# ==============================================================================
_LONG_CONTEXT_HEADER = """# ENTERPRISE DISTRIBUTED SYSTEM AUDIT TRACE LOG
System ID: CLUSTER-GRID-OMEGA-99 | Execution Scope: 16k Deep-Context Retrieval Verification

--- BEGIN AUDIT LOG STREAM ---
"""

_LONG_CONTEXT_LOG_SEGMENT_A = [
...
PROMPT_LONG_CONTEXT_RETRIEVAL = (
    _LONG_CONTEXT_HEADER
    + "\n".join(_LONG_CONTEXT_LOG_SEGMENT_A * 26)
    + _NEEDLE_1
    + "\n".join(_LONG_CONTEXT_LOG_SEGMENT_B * 26)
    + _NEEDLE_2
    + "\n".join(_LONG_CONTEXT_LOG_SEGMENT_C * 26)
    + _NEEDLE_3
    + _LONG_CONTEXT_FOOTER
)
...
# ==============================================================================
# WORKLOAD 9: SUMMARIZATION & DISTILLATION (~4,500 prompt tokens / ~300 output tokens)
# Purpose: Dense document context reduction measuring prefill efficiency & distillation.
# ==============================================================================
_SUMMARIZATION_BODY = """# ANNUAL INFRASTRUCTURE SCALABILITY, RELIABILITY & FINANCIAL AUDIT REPORT
...
PROMPT_SUMMARIZATION_DISTILL = _SUMMARIZATION_BODY * 3 + "\n" + _SUMMARIZATION_TASK


# ==============================================================================
# WORKLOAD 10: STRUCTURED JSON & GRAMMAR (~600 prompt tokens / ~300 output tokens)
# Purpose: Guided grammar decoding measuring schema validity compliance & decode speed.
# ==============================================================================
PROMPT_STRUCTURED_JSON = """Return a strictly valid JSON object representing a real-time cluster health and telemetry snapshot.
...
# ==============================================================================
# WORKLOAD 11: INTERACTIVE CONVERSATIONAL (~200 prompt tokens / ~150 output tokens)
# Purpose: Real-time conversational responsiveness, reading speed, and decode smoothness.
# ==============================================================================
PROMPT_CHAT_INTERACTIVE = (
    "As a Principal Distributed Systems Architect, explain the core technical trade-offs between Server-Sent Events (SSE), "
    "WebSockets, and HTTP/2 Long-Polling when building high-concurrency real-time streaming dashboards for LLM inference monitoring. "
    "Specifically evaluate: 1. TCP connection multiplexing and HTTP/2 flow control; 2. Proxy buffering, compression, and chunked transfer encoding; "
    "3. Automatic reconnection and state resumption backoff semantics; 4. Server-side memory overhead when sustaining 50,000 active concurrent streaming connections. "
    "Provide a concise, highly technical two-paragraph architectural recommendation."
)


# ==============================================================================
# WORKLOAD 12: FEW-SHOT IN-CONTEXT CLASSIFICATION (~1,200 prompt tokens / ~10 output tokens)
# Purpose: High-throughput in-context learning (ICL) classification and intent routing.
# ==============================================================================
PROMPT_FEWSHOT_CLASSIFICATION = """Classify incoming enterprise support escalations into strict JSON schema with fields:
...
# ==============================================================================
# WORKLOAD 13: MULTIMODAL VISION & OCR (~1,800 prompt tokens / ~200 output tokens)
# Purpose: Benchmark vision-language model (VLM) prefill latency and diagram analysis.
# ==============================================================================
_VISION_SPATIAL_TOPOLOGY = """[MULTIMODAL VISION TOKEN PAYLOAD: High-Resolution 4K Architecture Topology Diagram & Telemetry Dashboard]
...
PROMPT_MULTIMODAL_VISION = _VISION_SPATIAL_TOPOLOGY * 2 + "\n" + _VISION_TASK


# ==============================================================================
# WORKLOAD 14: MULTI-TURN SESSION CONTEXT (~2,500 prompt tokens / ~350 output tokens)
# Purpose: Accumulated session history measuring KV cache expansion & turn latency drift.
# ==============================================================================
_MULTITURN_DIALOG = """System: You are an expert Site Reliability Engineer collaborating on an active triage incident in a high-concurrency LLM inference platform.
...
PROMPT_MULTITURN_AGENTIC = _MULTITURN_DIALOG


# ==============================================================================
# WORKLOAD 15: PROMPT PREFIX CACHE REUSE (~3,200 prompt tokens / ~150 output tokens)
# Purpose: Static shared prefix measuring KV cache hit speedup ratio & TTFT reduction.
# ==============================================================================
_PREFIX_CACHE_STATIC_DOC = """[STATIC SYSTEM ARCHITECTURE & API REFERENCE DOCUMENTATION - PREFIX CACHE TARGET]
...
PROMPT_KV_CACHE_REUSE = _PREFIX_CACHE_STATIC_DOC * 5 + _PREFIX_CACHE_DYNAMIC_QUERY


# ==============================================================================
# WORKLOAD 16: CUSTOM WORKLOAD STUDIO DEFAULT (~500 prompt tokens / ~500 output tokens)
# Purpose: Comprehensive baseline prompt for user-configured custom experiments.
# ==============================================================================
PROMPT_CUSTOM = (
    "Provide a detailed technical evaluation of speculative decoding, chunked prefill, and paged KV memory architectures "
    "in modern LLM serving engines. Analyze the trade-offs between latency, GPU memory bandwidth utilization, and token throughput."
)


# ==============================================================================
# MASTER PROMPT PRESET DICTIONARY
# Maps all WorkloadPreset enums (including legacy aliases) to calibrated production prompts.
# ==============================================================================
PROMPT_PRESET_TEXT: dict[WorkloadPreset, str] = {
    # 1. Micro-Probe
    WorkloadPreset.RATE_LIMIT_PROBE: PROMPT_RATE_LIMIT_PROBE,
    # 2. Prefill / TTFT
    WorkloadPreset.PREFILL_TTFT: PROMPT_PREFILL_TTFT,
    # 3. Decode / Throughput
    WorkloadPreset.DECODE_THROUGHPUT: PROMPT_DECODE_THROUGHPUT,
    # 4. Reasoning / CoT
    WorkloadPreset.REASONING_COT: PROMPT_REASONING_COT,
    # 5. Agentic / Tool Calling
    WorkloadPreset.AGENTIC_TOOL_CALLING: PROMPT_AGENTIC_TOOL_CALLING,
    WorkloadPreset.TOOL_CALLING: PROMPT_AGENTIC_TOOL_CALLING,  # Legacy alias
    # 6. Code Generation
    WorkloadPreset.CODE_GENERATION: PROMPT_CODE_GENERATION,
    WorkloadPreset.CODE: PROMPT_CODE_GENERATION,  # Legacy alias
    # 7. RAG Synthesis
    WorkloadPreset.RAG_SYNTHESIS: PROMPT_RAG_SYNTHESIS,
    WorkloadPreset.RAG: PROMPT_RAG_SYNTHESIS,  # Legacy alias
    # 8. Long-Context Retrieval
    WorkloadPreset.LONG_CONTEXT_RETRIEVAL: PROMPT_LONG_CONTEXT_RETRIEVAL,
    WorkloadPreset.LONG_CONTEXT: PROMPT_LONG_CONTEXT_RETRIEVAL,  # Legacy alias
    # 9. Summarization & Distill
    WorkloadPreset.SUMMARIZATION_DISTILL: PROMPT_SUMMARIZATION_DISTILL,
    WorkloadPreset.SUMMARIZATION: PROMPT_SUMMARIZATION_DISTILL,  # Legacy alias
    # 10. Structured JSON
    WorkloadPreset.STRUCTURED_JSON: PROMPT_STRUCTURED_JSON,
    WorkloadPreset.JSON_SCHEMA: PROMPT_STRUCTURED_JSON,  # Legacy alias
    # 11. Interactive Chat
    WorkloadPreset.CHAT_INTERACTIVE: PROMPT_CHAT_INTERACTIVE,
    WorkloadPreset.CHAT: PROMPT_CHAT_INTERACTIVE,  # Legacy alias
    # 12. Few-Shot Classification
    WorkloadPreset.FEWSHOT_CLASSIFICATION: PROMPT_FEWSHOT_CLASSIFICATION,
    # 13. Multimodal Vision
    WorkloadPreset.MULTIMODAL_VISION: PROMPT_MULTIMODAL_VISION,
    WorkloadPreset.VISION: PROMPT_MULTIMODAL_VISION,  # Legacy alias
    # 14. Multi-Turn Session
    WorkloadPreset.MULTITURN_AGENTIC: PROMPT_MULTITURN_AGENTIC,
    # 15. Prefix Cache Reuse
    WorkloadPreset.KV_CACHE_REUSE: PROMPT_KV_CACHE_REUSE,
    # 16. Custom
    WorkloadPreset.CUSTOM: PROMPT_CUSTOM,
}


def get_preset_prompt(preset: WorkloadPreset | str) -> str:
    """Retrieve the production-grade prompt string for a given workload preset."""
    if isinstance(preset, str):
        try:
            preset = WorkloadPreset(preset)
        except ValueError:
            return PROMPT_CUSTOM
    return PROMPT_PRESET_TEXT.get(preset, PROMPT_CUSTOM)
