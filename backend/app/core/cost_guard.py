...
PRESET_TOKEN_PROFILES = {
    ...
    WorkloadPreset.LONG_CONTEXT: (16000, 500),
...
class CostGuard:
    ...
    @classmethod
    def register_dynamic_pricing(cls, model_id: str, prompt_price_per_1m: float, completion_price_per_1m: float) -> None:
        """Register dynamic pricing discovered from endpoint schema (e.g. OpenRouter)."""
        cls.DYNAMIC_PRICING[model_id.lower().strip()] = (prompt_price_per_1m, completion_price_per_1m)

    @classmethod
    def get_pricing(
        cls,
        model: str,
        custom_prompt: Optional[float] = None,
        custom_completion: Optional[float] = None,
    ) -> Tuple[float, float]:
        ...
        final_prompt = float(custom_prompt) if custom_prompt is not None and custom_prompt >= 0 else base_prompt
        final_completion = (
            float(custom_completion) if custom_completion is not None and custom_completion >= 0 else base_completion
        )
    ...
    @classmethod
    def is_spend_cap_exceeded(cls, current_spend: float, hard_spend_cap: Optional[float]) -> bool:
