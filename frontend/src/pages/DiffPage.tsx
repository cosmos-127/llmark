...
import { GitCompare, ArrowRight, TrendingUp, TrendingDown, Minus, Download, Copy, Check, Sparkles, Layers, Zap } from "lucide-react";
    ...
    <TooltipProvider>
      <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="p-5 pt-0">
              <Select value={runAId} onValueChange={setRunAId}>
                <SelectTrigger className="focus:border-[#612D53] dark:focus:border-[#7E3B6C] font-sans">
                  <SelectValue placeholder="Select baseline run A..." />
                </SelectTrigger>
                <SelectContent>
                  {runs?.map((r) => (
                    <SelectItem key={r.id} value={r.id}>
                      {r.name} ({r.model} • {r.vendor}) — P95: {r.ttft_p95.toFixed(1)}ms
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <Card>
            ...
            <CardContent className="p-5 pt-0">
              <Select value={runBId} onValueChange={setRunBId}>
                <SelectTrigger className="focus:border-[#853953] dark:focus:border-[#A74B6A] font-sans">
        ...
        {!runAId || !runBId ? (
          ...
          <Card className="h-64 flex flex-col items-center justify-center text-xs text-[#2C2C2C]/60 dark:text-[#F3F4F4]/60 space-y-2 font-sans">
              ...
              <CardContent className="p-4 sm:p-5 flex flex-wrap items-center justify-between gap-4">
                <div className="space-y-1.5 font-sans">
                  <div className="flex items-center gap-3 text-sm font-medium text-[#2C2C2C] dark:text-[#F3F4F4]">
                    <Badge variant="violet" className="text-xs py-1 px-3">
                      {diffData.run_a_name}
                    </Badge>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button
                    ...
                    className="rounded-xl font-medium shadow-2xs"
                  >
                    {copiedMd ? <Check className="h-3.5 w-3.5 text-emerald-600 dark:text-emerald-400" /> : <Copy className="h-3.5 w-3.5 text-[#853953] dark:text-[#A74B6A]" />}
                    {copiedMd ? "Copied!" : "Copy summary table"}
                  </Button>
                </div>
              </CardContent>
            </Card>
                ...
                <TableBody>
                  {diffData.deltas.map((d: MetricDelta, idx: number) => (
                    <TableRow key={idx}>
                      ...
                      <TableCell className="py-3.5 px-3 text-[#2C2C2C]/70 dark:text-[#F3F4F4]/70 font-mono">{d.run_a_value}</TableCell>
                      <TableCell className="py-3.5 px-3 text-[#2C2C2C] dark:text-[#F3F4F4] font-bold font-mono">{d.run_b_value}</TableCell>
                      <TableCell className="py-3.5 px-3 text-[#2C2C2C]/70 dark:text-[#F3F4F4]/70 font-mono">
                      ...
                      <TableCell className="py-3.5 px-5 text-right">
                        <Badge
                          ...
                          className="gap-1.5 py-1 px-3 text-xs font-medium font-mono"
                        >
                          {d.delta_pct === 0 ? (
