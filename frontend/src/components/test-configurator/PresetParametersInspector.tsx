...
export const PresetParametersInspector: React.FC<PresetParametersInspectorProps> = ({
  ...
  if (!preset || !config.workload_preset) {
    ...
    return (
      <Card className="rounded-2xl border border-[#2C2C2C]/10 dark:border-white/10 shadow-2xs bg-white dark:bg-[#0F0F13] overflow-hidden">
        <CardHeader className="p-4 pb-3 border-b border-[#2C2C2C]/5 dark:border-white/5 bg-[#F3F4F4]/30 dark:bg-[#0B0B0E]/50">
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div className="flex items-center gap-2.5">
              <div className="p-2 rounded-xl bg-[#853953]/10 text-[#853953] dark:text-[#F06A9A] animate-pulse">
                <Sliders className="h-4.5 w-4.5" />
              </div>
              <div>
                <CardTitle className="text-xs sm:text-sm font-bold font-sans text-[#2C2C2C] dark:text-white flex items-center gap-2">
        ...
        <CardContent className="p-4 space-y-3 font-sans">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {shimmerCards.map((card, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="p-3 rounded-xl border border-[#2C2C2C]/10 dark:border-white/10 bg-white/60 dark:bg-[#121217]/60 space-y-2.5 transition-all"
                >
                  <div className="flex items-center justify-between gap-1">
                    <span className="text-xs text-[#2C2C2C]/60 dark:text-slate-400 font-semibold flex items-center gap-1.5">
                      <Icon className="h-3.5 w-3.5 text-[#853953]/50 dark:text-[#F06A9A]/50 shrink-0" />
                      {card.label}
                    </span>
                  ...
                  <div className="flex items-baseline justify-between pt-0.5">
                    <span className="text-sm font-bold text-[#2C2C2C]/40 dark:text-slate-600 font-mono">
                      {card.defaultVal}
                    </span>
                    <div className="h-3 w-16 bg-slate-200/60 dark:bg-white/5 rounded animate-pulse" />
                  </div>

                  <div className="space-y-1 pt-1 border-t border-[#2C2C2C]/5 dark:border-white/5">
          ...
          {/* Shimmer Footnote */}
          <div className="p-3 rounded-xl bg-[#F3F4F4]/40 dark:bg-[#0B0B0E] border border-[#2C2C2C]/5 dark:border-white/5 flex flex-wrap items-center justify-between gap-2.5 text-xs">
            <div className="flex items-center gap-2">
              <span className="text-[11px] text-[#2C2C2C]/50 dark:text-slate-500 font-medium">Focus:</span>
              <div className="h-4 w-36 bg-slate-200/70 dark:bg-white/5 rounded animate-pulse" />
            </div>

            <div className="flex items-center gap-1.5 flex-wrap">
              <span className="text-[11px] text-[#2C2C2C]/50 dark:text-slate-500 font-medium">Metrics:</span>
  ...
  return (
    <Card className="rounded-2xl border border-[#2C2C2C]/10 dark:border-white/10 shadow-2xs bg-white dark:bg-[#0F0F13] overflow-hidden">
      <CardHeader className="p-4 pb-3 border-b border-[#2C2C2C]/5 dark:border-white/5 bg-[#F3F4F4]/30 dark:bg-[#0B0B0E]/50">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            <div className="p-2 rounded-xl bg-[#853953]/10 text-[#853953] dark:text-[#F06A9A]">
              <Sliders className="h-4.5 w-4.5" />
            </div>
            <div>
              <CardTitle className="text-xs sm:text-sm font-bold font-sans text-[#2C2C2C] dark:text-white flex items-center gap-2">
      ...
      <CardContent className="p-4 space-y-3 font-sans">
            ...
            return (
              <div
                ...
                className={`p-3 rounded-xl border transition-all flex flex-col justify-between ${
                  param.isLocked
                    ? "bg-[#F3F4F4]/50 dark:bg-[#15151C]/80 border-amber-500/20 dark:border-amber-500/25"
                    : "bg-white dark:bg-[#121217] border-[#2C2C2C]/10 dark:border-white/10"
                }`}
              >
                <div className="space-y-1.5">
                  <div className="flex items-center justify-between gap-1">
                    <span className="text-xs text-[#2C2C2C]/70 dark:text-slate-300 font-semibold flex items-center gap-1.5 truncate">
                      <Icon className="h-3.5 w-3.5 text-[#853953] dark:text-[#F06A9A] shrink-0" />
                      {param.label}
                    </span>
                    {param.isLocked ? (
                      <Badge
                  ...
                  <div className="flex items-baseline justify-between pt-0.5">
                    <span className="text-sm font-bold text-[#2C2C2C] dark:text-white tabular-nums">
                      {param.value}
                    </span>
                    <span className="text-[10px] text-[#2C2C2C]/50 dark:text-slate-400 font-medium truncate ml-2">
                      {param.statusLabel}
                    </span>
                  </div>
                </div>

                <p className="text-[11px] text-[#2C2C2C]/65 dark:text-slate-300 mt-2 leading-relaxed border-t border-[#2C2C2C]/5 dark:border-white/5 pt-1.5">
                  {param.impact}
                </p>
              </div>
            );
        ...
        <div className="p-3 rounded-xl bg-[#F3F4F4]/40 dark:bg-[#0B0B0E] border border-[#2C2C2C]/5 dark:border-white/5 flex flex-wrap items-center justify-between gap-2.5 text-xs">
          <div className="flex items-center gap-2">
            <span className="text-[11px] text-[#2C2C2C]/60 dark:text-slate-400 font-medium">Focus:</span>
            <span className="font-semibold text-xs text-[#853953] dark:text-[#F06A9A]">
              {promptDetails?.targetStressDimension || preset.tag}
            </span>
          </div>

          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[11px] text-[#2C2C2C]/60 dark:text-slate-400 font-medium">Metrics:</span>
