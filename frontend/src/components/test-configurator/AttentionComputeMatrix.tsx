    ...
    <div className="rounded-2xl border border-[#0F172A]/10 dark:border-white/10 bg-white dark:bg-[#111827] p-5 sm:p-6 space-y-5 shadow-xs">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-center p-4 sm:p-5 rounded-2xl bg-[#F1F5F9]/70 dark:bg-[#1E293B] border border-[#0F172A]/10 select-none">
        ...
        <div className="lg:col-span-5 flex flex-col items-center justify-center space-y-3">
          <div className="p-3 rounded-2xl bg-white dark:bg-[#111827] border border-[#0F172A]/10 shadow-inner">
                  ...
                  return (
                    <motion.div
                      ...
                      className={`h-6 w-6 rounded-sm flex items-center justify-center transition-all cursor-pointer ${
                        isCausalMasked
                          ? "bg-[#0F172A]/5 dark:bg-[#F1F5F9]/5 opacity-15 cursor-not-allowed"
                          : isDecodeCell
                          ? "bg-[#1D4ED8] dark:bg-[#60A5FA] text-white shadow-2xs hover:brightness-110"
                          : "bg-[#2563EB] dark:bg-[#3B82F6] text-white shadow-2xs hover:brightness-110"
          ...
          <div className="flex items-center justify-center gap-4 text-xs font-sans text-[#0F172A]/70 dark:text-slate-300">
            ...
            <span className="flex items-center gap-1.5 opacity-50">
              <span className="h-2.5 w-2.5 rounded-xs bg-[#0F172A]/20 dark:bg-[#F1F5F9]/20" />
          ...
          {hoveredCell && (
            <div className="text-xs font-sans text-center bg-white dark:bg-[#111827] border border-[#0F172A]/10 rounded-lg px-3 py-1 text-[#0F172A] dark:text-white">
              ...
              {hoveredCell.k > hoveredCell.q ? (
                ...
                <span className="text-[#2563EB] dark:text-[#60A5FA] font-semibold ml-1.5">(Parallel GEMM)</span>
              ) : (
                <span className="text-[#1D4ED8] dark:text-[#38BDF8] font-semibold ml-1.5">(KV Cache GEMV)</span>
              )}
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-7 space-y-3.5 text-xs">
          {/* Prefill Phase Row */}
          <div className="p-3.5 rounded-xl bg-white dark:bg-[#111827] border border-[#0F172A]/10 space-y-2">
            <div className="flex items-center justify-between">
              <span className="font-bold text-[#2563EB] dark:text-[#60A5FA] flex items-center gap-2 text-xs">
                <Cpu className="h-4 w-4" />
                <span>1. Prefill Phase (Prompt Ingestion)</span>
              </span>
              <Badge variant="outline" className="font-sans font-bold text-[#2563EB] dark:text-[#60A5FA] bg-[#2563EB]/10 dark:bg-[#3B82F6]/15 border-[#2563EB]/30 dark:border-[#3B82F6]/40 text-[11px]">
                Compute-Bound (Tensor Cores)
              </Badge>
            </div>
            <p className="text-xs text-[#0F172A]/75 dark:text-white/75 leading-relaxed">
              All <strong className="text-[#0F172A] dark:text-white">{promptTokens.toLocaleString()} prompt tokens</strong> are multiplied in a single parallel <strong className="text-[#0F172A] dark:text-white">GEMM</strong> (General Matrix Multiply). High arithmetic intensity (<MathFormula math="I \gg 100 \text{ FLOPs/Byte}" />) fully saturates GPU Tensor Cores. Dictates <strong className="text-[#2563EB] dark:text-[#60A5FA]">Time to First Token (TTFT)</strong>.
            </p>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-white dark:bg-[#111827] border border-[#0F172A]/10 space-y-2">
            <div className="flex items-center justify-between">
              <span className="font-bold text-[#1D4ED8] dark:text-[#38BDF8] flex items-center gap-2 text-xs">
            ...
            <p className="text-xs text-[#0F172A]/75 dark:text-white/75 leading-relaxed">
              To generate each single output token (1 at a time), the GPU executes a <strong className="text-[#0F172A] dark:text-white">GEMV</strong> (Matrix-Vector Multiply). The entire multi-gigabyte model weights must be re-read from GPU HBM into SRAM for <em>each generated token</em>. Low arithmetic intensity (<MathFormula math="I \approx 1\text{ FLOP/Byte}" />) bottlenecks generation on memory bandwidth, dictating <strong className="text-[#1D4ED8] dark:text-[#38BDF8]">Time Per Output Token (TPOT / ITL)</strong>.
            </p>
          </div>
        </div>
      </div>
