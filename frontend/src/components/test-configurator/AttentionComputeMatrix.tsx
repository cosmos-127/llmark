...
export const AttentionComputeMatrix: React.FC<AttentionComputeMatrixProps> = ({
  ...
  return (
    <div className="rounded-2xl border border-[#2C2C2C]/10 dark:border-white/10 bg-white dark:bg-[#0F0F13] p-5 sm:p-6 space-y-5 shadow-xs">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-5 items-center p-4 sm:p-5 rounded-2xl bg-[#F3F4F4]/70 dark:bg-[#14141B] border border-[#2C2C2C]/10 select-none">
        {/* Left: 2D Triangular Attention Heatmap Grid */}
        <div className="lg:col-span-5 flex flex-col items-center justify-center space-y-3">
          <div className="p-3 rounded-2xl bg-white dark:bg-[#0F0F13] border border-[#2C2C2C]/10 shadow-inner">
                    ...
                    <motion.div
                      ...
                      className={`h-6 w-6 rounded-sm flex items-center justify-center transition-all cursor-pointer ${
                        isCausalMasked
                          ...
                          : "bg-[#853953] dark:bg-[#D84577] text-white shadow-2xs hover:brightness-110"
          ...
          <div className="flex items-center justify-center gap-4 text-xs font-sans text-[#2C2C2C]/70 dark:text-slate-300">
          ...
          {hoveredCell && (
            <div className="text-xs font-sans text-center bg-white dark:bg-[#0F0F13] border border-[#2C2C2C]/10 rounded-lg px-3 py-1 text-[#2C2C2C] dark:text-white">
              ...
              {hoveredCell.k > hoveredCell.q ? (
                ...
                <span className="text-[#853953] dark:text-[#F06A9A] font-semibold ml-1.5">(Parallel GEMM)</span>
              ) : (
                <span className="text-[#612D53] dark:text-[#E270BB] font-semibold ml-1.5">(KV Cache GEMV)</span>
              )}
            </div>
          )}
        </div>
        ...
        <div className="lg:col-span-7 space-y-3.5 text-xs">
          {/* Prefill Phase Row */}
          <div className="p-3.5 rounded-xl bg-white dark:bg-[#0F0F13] border border-[#2C2C2C]/10 space-y-2">
            <div className="flex items-center justify-between">
              <span className="font-bold text-[#853953] dark:text-[#F06A9A] flex items-center gap-2 text-xs">
                <Cpu className="h-4 w-4" />
                <span>1. Prefill Phase (Prompt Ingestion)</span>
              </span>
              <Badge variant="outline" className="font-sans font-bold text-[#853953] dark:text-[#F06A9A] bg-[#853953]/10 dark:bg-[#D84577]/15 border-[#853953]/30 dark:border-[#E05284]/40 text-[11px]">
                Compute-Bound (Tensor Cores)
              </Badge>
            </div>
            <p className="text-xs text-[#2C2C2C]/75 dark:text-white/75 leading-relaxed">
              All <strong className="text-[#2C2C2C] dark:text-white">{promptTokens.toLocaleString()} prompt tokens</strong> are multiplied in a single parallel <strong className="text-[#2C2C2C] dark:text-white">GEMM</strong> (General Matrix Multiply). High arithmetic intensity (<MathFormula math="I \gg 100 \text{ FLOPs/Byte}" />) fully saturates GPU Tensor Cores. Dictates <strong className="text-[#853953] dark:text-[#F06A9A]">Time to First Token (TTFT)</strong>.
            </p>
          </div>
          ...
          <div className="p-3.5 rounded-xl bg-white dark:bg-[#0F0F13] border border-[#2C2C2C]/10 space-y-2">
            <div className="flex items-center justify-between">
              <span className="font-bold text-[#612D53] dark:text-[#E270BB] flex items-center gap-2 text-xs">
            ...
            <p className="text-xs text-[#2C2C2C]/75 dark:text-white/75 leading-relaxed">
              To generate each single output token (1 at a time), the GPU executes a <strong className="text-[#2C2C2C] dark:text-white">GEMV</strong> (Matrix-Vector Multiply). The entire multi-gigabyte model weights must be re-read from GPU HBM into SRAM for <em>each generated token</em>. Low arithmetic intensity (<MathFormula math="I \approx 1\text{ FLOP/Byte}" />) bottlenecks generation on memory bandwidth, dictating <strong className="text-[#612D53] dark:text-[#E270BB]">Time Per Output Token (TPOT / ITL)</strong>.
            </p>
          </div>
        </div>
      </div>
