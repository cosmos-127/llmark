...
export const MetricCards: React.FC<MetricCardsProps> = ({ snapshot, workloadPreset }) => {
  ...
  const kneeBanner = (snapshot?.saturation_knee_detected || snapshot?.saturation_knee_concurrency) ? (
    <div className="col-span-full mb-1 p-3.5 rounded-xl bg-gradient-to-r from-[var(--brand-primary-light)] via-[var(--brand-primary-light)]/50 to-transparent border border-[var(--brand-primary-border)] flex items-center justify-between gap-3">
      <div className="flex items-center gap-2.5">
        <div className="p-2 rounded-lg bg-[var(--brand-primary)] text-[var(--text-inverse)] shadow-2xs">
          <Gauge className="h-4 w-4" />
        </div>
        <div>
          <div className="text-xs font-bold text-[var(--brand-primary)] flex items-center gap-1.5">
            Saturation Knee Inflection Discovered
            <Badge variant="destructive" className="text-[10px] py-0 h-4">Inflection Point</Badge>
          </div>
          <p className="text-[11px] text-[var(--text-body)] pt-0.5">
      ...
      <Badge variant="outline" className="text-xs font-semibold border-[var(--brand-primary-border)] text-[var(--brand-primary)] tabular-nums whitespace-nowrap">
